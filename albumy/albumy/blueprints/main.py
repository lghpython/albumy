...
from flask import Blueprint, redirect, render_template, request, current_app, send_from_directory, flash, url_for, abort
from flask_login import login_required, current_user
from flask_dropzone import random_filename
from albumy.decorators import admin_required, permission_required, confirm_required
from albumy.models import Photo, Follow, Tag, User, User, Notification, Comment, Collect
from albumy.forms.main import CommentForm, DescriptionForm, TagForm
from albumy.extensions import db
from albumy.notifications import push_comment_notification, push_collect_notification
from albumy.utils import redirect_back, resize_image, flash_errors
...
def index():
    if current_user.is_authenticated:
        ...
        pagination = Photo.query.join(Follow, Follow.followed_id==Photo.id)\
...
def search():
    ...
    if q == '':
        flash('请输入关键字关于 photo、user、tag 进行搜索', 'warning')
...
def show_notifications():
    ...
    filter_rules = request.args.get('filter')
    if filter_rules == 'unread':
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        photo = Photo(
            ...
            author=current_user._get_current_object(), # 获取真实的用户对象建立关系
        )
...
def photo_previous(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    photo_p = Photo.query.with_parent(photo.author).filter(Photo.id>photo.id)\
...
def edit_description(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    if current_user != photo.author and not current_app.can('MODERATE'):
        abort(403)
    form = DescriptionForm()
    if form.validate_on_submit():
        photo.description = form.description.data
        db.session.commit()
        flash('更改描述成功', 'success')
    flash_errors(form)
    return redirect(url_for('.show_photo', photo_id=photo_id))
