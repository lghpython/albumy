...
def send_mail(subject, to, template, **kwargs):
    message = Message(current_app.config['ALBUMY_MAIL_SUBJECT_PRIFEX'] + subject, recipients=[to])
...
def send_confirm_account_mail(user,token, to=None):
    send_mail(subject='Active Account', to=to or user.email, template='emails/confirm', user=user, token=token)
